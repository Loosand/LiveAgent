    ...
    content: [
      {
        ...
        text: [
          ...
          "",
          "这套示例用于同时检查段落节奏、标题层级、列表缩进和复杂内容的阅读密度。",
          "",
...
const LIVEAGENT_OVERVIEW_MESSAGES: Message[] = [
  userMessage({
    id: "gallery-liveagent-overview-user",
    minute: 2,
    content: "请用一条结构完整的 Markdown 回复介绍当前 LiveAgent 项目。",
  }),
  assistantMessage({
    id: "gallery-liveagent-overview-assistant",
    minute: 3,
    contextTokens: 12_480,
    messageUsage: usage(10_920, 1_560, 4_200),
    content: [
      {
        type: "text",
        text: [
          "# LiveAgent — 本地优先的 AI Agent 桌面端",
          "",
          "## 一、项目定位",
          "",
          "**LiveAgent** 是一个以桌面端为本地执行核心的 AI Agent 应用。它把模型推理与文件系统、Shell、MCP、Skills、Memory 和自动化能力连接起来；桌面端可以独立工作，需要远程访问时，再通过 Go Gateway 与浏览器 WebUI 操作同一个本地 Agent 会话。",
          "",
          "> 核心原则：高权限能力和持久化都留在桌面端，Gateway 只负责认证、连接和协议中继。",
          "",
          "## 二、技术栈",
          "",
          "| 层级 | 当前技术 |",
          "| --- | --- |",
          "| **桌面 UI** | Tauri 2.11 · React 19.2 · TypeScript 7 |",
          "| **构建与样式** | Vite 8.0 · Tailwind CSS 4.2 · Base UI 1.6 |",
          "| **富文本渲染** | Streamdown 2.5 · KaTeX 0.18 · Mermaid · Monaco Editor 0.56 |",
          "| **Agent 运行时** | `@earendil-works/pi-ai` · `@earendil-works/pi-agent-core` |",
          "| **桌面后端** | Rust · Tokio · SQLite（rusqlite）· WebSocket |",
          "| **远程 Gateway** | Go 1.25 · HTTP · WebSocket · Protobuf |",
          "| **包管理** | pnpm 10.32 workspace |",
          "",
          "**同时支持：**",
          "",
          "- macOS、Windows 与 Linux 桌面端",
          "- Claude、Codex 与 Gemini 协议，以及自定义 Base URL",
          "- GUI 与 WebUI 复用同一套共享应用 UI",
          "",
          "## 三、仓库结构（Monorepo）",
          "",
          "```纯文本",
          "LiveAgent/",
          "├── crates/",
          "│   ├── agent-ui/             # GUI / WebUI 共享 React 应用 UI",
          "│   ├── agent-gui/            # React 桌面前端与 Tauri / Rust 后端",
          "│   ├── agent-gateway/        # Go Gateway、Protobuf 与嵌入式 WebUI",
          "│   └── virtual-core/         # 工作区内维护的虚拟列表核心",
          "├── docs/",
          "│   ├── architecture/         # 当前系统架构与进程边界",
          "│   ├── features/             # Chat、Tools、Memory、Skills / MCP",
          "│   ├── operations/           # 开发、测试与部署",
          "│   └── reference/            # 源码路径索引",
          "├── scripts/                  # 开发、检查与发布自动化",
          "├── Makefile                  # 常用开发入口",
          ...
          "### 架构约束",
          "",
          "1. 桌面 GUI / Tauri 是工具执行、设置、历史和记忆的真实来源。",
          "2. Gateway 不直接执行本地工具，也不访问用户文件系统或保存真实 Provider Key。",
          "3. GUI 与 WebUI 的公共页面只在 `crates/agent-ui` 维护一份。",
          "4. 平台差异通过各宿主的 `agent-ui-adapters/*` 或扩展注册表接入。",
          "5. 长对话由 Segment 与 Summary Checkpoint 持久化；短时断线由 Gateway 的有界事件窗口补齐。",
          "",
          "## 四、主要能力",
          "",
          "### 🧠 对话与模型",
          ...
          "- KaTeX 公式、Mermaid 图表与 Monaco 代码预览",
          "- 历史压缩、上下文检查点与国际化",
          "",
          "### 🔧 本地执行",
          "",
          ...
          "- Bash、可恢复长驻进程与终止控制",
          "- Sub-Agent 并行委派与隔离 worktree",
          "- 项目附加目录的独立读写授权",
          "",
          "### 🧩 扩展、记忆与自动化",
          "",
          "- stdio、HTTP 与 SSE MCP Server",
          "- Skills 安装、创建、校验、打包与按需加载",
          "- Markdown 事实源与 SQLite FTS 记忆检索",
          "- Bash、HTTP 与 Prompt 类型的定时任务",
          "",
          "## 五、本地开发",
          "",
          "```Bash",
          ...
          "",
          "make dev                    # 启动 Tauri 桌面开发环境",
          "make dev-stack              # 启动 Gateway、WebUI 与桌面端",
          "pnpm --filter liveagent dev:chat-gallery",
          "```",
          "",
          "> ⚠️ **共享 UI 约束**",
          ">",
          "> - 公共 JSX / CSS 放在 `crates/agent-ui`。",
          "> - 宿主差异放在 adapter 或扩展注册表中。",
          "> - 修改共享聊天 UI 后，同时验证桌面 GUI 与 WebUI。",
          "",
          "**常用检查：**",
          "",
          "```Bash",
          ...
          "## 六、文档入口",
          "",
          "- [架构总览](/Volumes/ssd/LiveAgent/docs/architecture/overview.md)",
          "- [Chat Runtime](/Volumes/ssd/LiveAgent/docs/features/chat-runtime.md)",
          "- [工具系统](/Volumes/ssd/LiveAgent/docs/features/tools.md)",
          "- [Skills 与 MCP](/Volumes/ssd/LiveAgent/docs/features/skills-and-mcp.md)",
          "- [开发与测试](/Volumes/ssd/LiveAgent/docs/operations/development.md)",
          "- [源码索引](/Volumes/ssd/LiveAgent/docs/reference/source-map.md)",
          "",
          "---",
          "",
          "## 一句话总结",
          "",
          "> **LiveAgent 是一个以桌面端为权限与数据真相源、同时支持浏览器远程操控的本地优先 AI Agent。**",
          "",
          "| 项目状态 | 当前值 |",
          "| --- | --- |",
          "| 桌面应用版本 | `1.3.0-dev.0` |",
          "| 桌面前端开发端口 | `1420` |",
          "| Gateway / WebUI 默认端口 | `50052` / `5173` |",
          "| License | MIT |",
        ].join("\n"),
      },
    ],
...
export const CHAT_GALLERY_FIXTURES = {
  ...
  liveAgentOverview: {
    title: "LiveAgent 项目介绍",
    description: "按桌面阅读基准组织的真实项目介绍，覆盖标题、表格、代码、引用和列表。",
    messages: LIVEAGENT_OVERVIEW_MESSAGES,
    tags: ["markdown", "reference", "project-overview"],
  },
  attachments: {
    title: "用户附件",
