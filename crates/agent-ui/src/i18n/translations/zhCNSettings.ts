export const ZH_CN_SETTINGS_TRANSLATIONS = {
  ...
  "settings.providerDirectoryEyebrow": "设置",
  "settings.providerSearchPlaceholder": "搜索供应商",
  "settings.providerAllConnections": "全部连接",
  "settings.providerAllConnectionsDesc": "集中管理不同协议的模型连接、模型目录和请求能力。",
  "settings.providerEnabledGroup": "已启用",
  "settings.providerInactiveGroup": "未启用",
  "settings.providerAvailableGroup": "可添加连接",
  "settings.providerAddVendorConnection": "添加 {vendor} 连接",
  "settings.providerNoMatches": "没有匹配的供应商",
  "settings.providerNoMatchesHint": "换一个名称、地址或协议关键词试试。",
  "settings.providerConfiguredStatus": "已配置",
  "settings.providerPendingStatus": "待配置",
  "settings.providerTemplateHint": "创建一个兼容连接，并配置 API 地址、密钥与可用模型。",
  "settings.providerBackToOverview": "返回全部连接",
  "settings.providerDetailEditHint": "配置连接、请求行为、模型目录和用量查询。",
  "settings.providerDetailCreateHint": "创建一个新的兼容连接并配置可用模型。",
  "settings.providerConnectionFormHint": "连接信息用于识别服务、建立请求并同步模型目录。",
  "settings.providerNameHint": "用于在模型选择器和供应商目录中区分不同连接。",
  "settings.providerBaseUrlHint": "填写官方 API、代理、中转或兼容服务地址。",
  "settings.providerApiKeyHint": "输入服务提供的密钥；保存后不会在界面中明文展示。",
  "settings.providerApiKeyConfiguredHint": "密钥已保存；保留当前内容不会替换，输入新值可更新。",
  "settings.providerModelsSectionHint": "从模型目录同步，或手动补充完整模型 ID 并选择启用状态。",
  "settings.providerRequestFormHint": "配置代理、流式重试、提示词缓存和自定义请求头。",
  "settings.providerUsageFormHint": "可选配置余额或套餐查询；关闭时不会发起额外请求。",
  "settings.providerUsageDisabledHint":
    "当前连接只用于模型请求。启用后，才会显示查询方式、凭证与测试配置。",
