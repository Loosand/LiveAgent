...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Backdrop
    ...
    className={cn(
      "layer-modal fixed inset-0 bg-overlay/40 backdrop-blur-xs transition-opacity duration-200 data-[ending-style]:opacity-0 data-[starting-style]:opacity-0 motion-reduce:transition-none",
      className,
    )}
    {...props}
  />
  ...
  (
    ...
    <SheetPortal>
      ...
      <SheetPrimitive.Popup
        ...
        data-side={side}
        className={cn(
          "layer-modal fixed flex max-h-full min-h-0 min-w-0 flex-col overflow-hidden bg-popover text-popover-foreground shadow-overlay outline-none transition-[transform,opacity] duration-200 ease-out data-[ending-style]:opacity-0 data-[starting-style]:opacity-0 motion-reduce:transition-none",
          side === "top" &&
          ...
          variant === "inset" && side === "right" && "inset-y-4 right-4 rounded-overlay border",
          variant === "inset" && side === "left" && "inset-y-4 left-4 rounded-overlay border",
          variant === "inset" && side === "top" && "inset-x-4 top-4 rounded-overlay border",
          variant === "inset" && side === "bottom" && "inset-x-4 bottom-4 rounded-overlay border",
          className,
        )}
        {...props}
      >
