...
export function Field(props: { label: string; value: string }) {
  return (
    <div className="flex gap-2 border-b border-border py-1 last:border-0">
...
export function TextBlock(props: { value: string | undefined; t: Translate; language?: string }) {
  ...
  return (
    <pre
      ...
      className="max-h-full whitespace-pre-wrap break-words rounded-sm bg-muted/40 p-2 font-mono text-xs leading-relaxed"
    >
      {props.value}
    </pre>
  );
      ...
      {props.blocks.map((block, index) => (
        <section key={`${block.type}:${block.callId ?? index}`} className="space-y-1">
          ...
          {block.filePath && block.fileSource && props.onOpenFileLink ? (
            <button
              type="button"
              className="max-w-full truncate rounded-sm border border-border px-2 py-1 text-left text-xs font-medium text-primary hover:bg-muted/60"
          ...
          {block.imageSrc ? (
            <img
              ...
              loading="lazy"
              className="max-h-72 max-w-full rounded-sm border border-border object-contain"
            />
  ...
  return (
    <div className="space-y-2">
      ...
      <button
        ...
        className="rounded-sm border border-border px-2 py-0.5 text-xs hover:bg-muted/60"
      >
