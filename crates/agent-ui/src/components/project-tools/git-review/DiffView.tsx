    ...
    <div ref={containerRef} className="border-b border-border/60 last:border-b-0">
      ...
      {!visible ? (
        ...
        <pre
          className={cn(
            GIT_REVIEW_TRANSIENT_SCROLLBAR_CLASS,
            "git-review-diff-selectable-content max-h-26rem select-text overflow-auto px-3 py-3 text-scaled-11px leading-relaxed text-muted-foreground",
          )}
    ...
    >
      ...
      {!error && !showLoadingState && diff?.patch.trim() && patchChunks.length === 0 ? (
        <pre
          ...
          className={cn(
            GIT_REVIEW_TRANSIENT_SCROLLBAR_CLASS,
            "git-review-diff-selectable-content min-h-0 flex-1 select-text overflow-auto px-3 py-3 text-scaled-11px leading-relaxed text-muted-foreground",
          )}
          onScroll={handleOverlayScroll}
        >
          {diff.patch}
      ...
      {selectionContextMenu
        ? createPortal(
            <div
              ref={contextMenuRef}
              role="menu"
              className="editor-context-menu layer-popover fixed w-max min-w-9p5rem max-w-viewport-inset-1p5rem select-none overflow-hidden rounded-xl border border-border/60 bg-popover/80 p-1 text-popover-foreground shadow-2xl ring-1 ring-black/[0.03] backdrop-blur-xl dark:ring-white/[0.06]"
              style={{
