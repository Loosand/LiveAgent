  ...
  return (
    <div
      className={cn(
        ...
        "tool-expand relative overflow-hidden rounded-2xl border border-border bg-background/80 dark:bg-white/5",
        // 只有仍可拍板的计划值得从转录里浮起来;已落定的一律回落成安静的历史文档。
        tone === "pending" ? "shadow-control" : "",
      )}
    >
      ...
      <span
        ...
        className={cn(
          ...
          tone === "approved"
            ...
            : tone === "pending"
              ? "bg-gradient-to-b from-info via-info to-activity shadow-control"
      ...
      <div className="flex items-center gap-2 border-b border-border px-4 py-2">
        ...
        />
        <span className="text-xs font-medium tracking-[0.01em] text-foreground/90">
          {t("chat.planMode.cardTitle")}
        </span>

        {/* 计划很长时按钮会落在视口外,表头状态让人不用滚到底也知道这份计划的处境。 */}
        <span className="ml-auto inline-flex shrink-0 items-center gap-2 text-xs leading-none text-muted-foreground">
          {tone === "approved" ? (
            <>
            ...
            <>
              <span className="h-1.5 w-1.5 rounded-full border border-input" />
      ...
      {canApprove ? (
        <div className="flex flex-wrap items-center gap-x-2 gap-y-2 border-t border-border bg-foreground/0 px-4 py-2 dark:bg-white/0">
          <button
            type="button"
            disabled={submitting}
            onClick={() => void approve()}
            className="group/approve inline-flex h-8 shrink-0 items-center gap-2 rounded-lg bg-primary px-4 text-xs font-medium text-primary-foreground shadow-control transition-[background-color,transform,box-shadow] duration-150 ease-out hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40 focus-visible:ring-offset-1 focus-visible:ring-offset-background active:scale-[0.98] disabled:pointer-events-none disabled:opacity-40 motion-reduce:transition-none motion-reduce:active:scale-100"
          >
            {submitting ? (
          ...
          {errorText ? (
            <span className="min-w-0 flex-1 text-xs leading-[1.5] text-[hsl(var(--chat-error))]">
              {errorText}
            </span>
          ) : null}
        </div>
